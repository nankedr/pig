...
`ProcessTerminal.RunCommand` 继承注入终端的输入输出。darwin/linux 使用独立进程组；有控制终端时将前台交给编辑器，退出后交回原进程组，并无条件恢复启动前的 termios，防止异常退出的编辑器泄漏 raw mode。Context 取消杀死并等待编辑器，清理同组子进程。核心保持无 CGO；其他平台显式返回 NotImplemented，六平台运行验收留在 M13。
