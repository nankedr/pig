...
Issue #69 沿用 ADR-0006：接收端立即验证精简事件并维护 Stream Outcome，FIFO 只排入精简事件，消费方调用 `Next` 时才重建该事件的不可变 partial；`Result` 独立完成并可并发重复读取，不共享 Pi 的可变消息引用。`ProxyStreamOptions.Fetch` 是不进入请求 JSON 的 Go transport 注入点，`AssistantMessageEventStream()` 将消费端包装接入现有 Agent SDK；请求保持固定 Pi 的 `/api/stream`、Bearer 认证及 12 项可序列化选项，`maxRetryDelayMs` 交给远端 Provider，本地不重试生成请求。
