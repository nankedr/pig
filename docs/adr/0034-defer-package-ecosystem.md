...
2026-09-16，#107：本地扩展发现与执行分离，新增只含数据的入口查询；自动发现遵循固定基线的文件/index 规则，显式目录也只扫描本地入口。忽略 manifest，不登记包，因此显式入口的 origin 标记为 top-level（Pi PackageManager 原始记录为 package）。普通本地资源会话可继续运行，诊断明确扩展未执行；显式执行请求仍返回未实现错误。该边界不冻结语言、回调或加载 ABI。
