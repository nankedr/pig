...
会话 JSON 用 base64 隔离 script 标签上下文。固定脚本通过 SHA-256 CSP 执行，内联事件改为固定监听器；read 行号参数和 Bash 退出码补 HTML 转义，导出前校验 Bash 字段及助手错误字段。禁止资源和连接网络，链接由用户显式点击打开。原始 Markdown HTML 按字面显示，URL 经过 scheme 白名单与属性转义。CSP 是第二道防线，浏览器测试另行断言无注入节点/事件属性、无攻击执行、无自动 HTTP 请求。

兼容边界：消息与 ToolResult 的图片块明确失败，Markdown 图片显示 M12 提示；不声称图片已经实现。扩展 renderer 在 M7，普通自定义 Tool 仍显示参数 JSON 与文本结果，不执行扩展代码或接受预渲染 HTML。Issue #104 已接入 M5 本地自定义主题及内置 dark/light；所有颜色经严格验证后进入 CSS，主题和 export 配色不接受任意样式或 HTML。未知条目、角色和内容块明确失败；合法但非显示的 custom/settings/label 条目依固定 Pi 保留在树和嵌入数据。独立迁移、SDK ExportToJSONL、托管分享和 M4 freeze/release 不在此切片完成声明中。
