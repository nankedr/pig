...
此切片保持 partial：ICU 的中文/泰语等词典分词、不同 Unicode 版本与终端宽度差异、完整 Kitty/modifyOtherKeys 协议、可配置键绑定、自动补全、复杂粘贴占位符跨视觉行的所有跳转组合仍未声明全面对等。自动补全已由 [M6.7](m6-autocomplete.md) 接续交付。完整 TUI/Markdown、scrollback、fullscreen、图片、扩展运行时、认证、包生态及六平台运行验收沿用后续里程碑边界。
