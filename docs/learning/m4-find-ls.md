...
实现入口是 `codingagent/find_tool.go` 和 `codingagent/ls_tool.go`。两个 definition 共享 Tool 执行体，通过 Agent dispatcher 校验参数并将错误送回模型；自定义 FindOperations/LsOperations 可连接远程文件系统。context 取消会阻止后续操作，fd 子进程被终止并回收。同步自定义操作需主动响应 context。
...
兼容分支的限制见 [ADR-0022](../adr/0022-find-ls-platform-contract.md)：自定义 FindGlobOptions 的 limit 仍为 int，其他 locale 排序和六平台运行验证待 M13，交互渲染与扩展宿主待 M7。Pi 的 fd 行式输出会截断换行文件名、trim 名称两端空白；不会声称这些名称可可靠回读。公开能力状态与复现证据登记于 `contract:codingagent/find-ls`。
