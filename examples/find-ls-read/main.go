...
func run() error {
	cwd, err := os.MkdirTemp("", "pig-find-ls-read-")
	if err != nil {
		return err
	}
	defer os.RemoveAll(cwd)
	if err := os.WriteFile(filepath.Join(cwd, "hello.txt"), []byte("你好🙂"), 0600); err != nil {
		return err
	}
	...
	for _, call := range []ai.ToolCall{
		...
		{Type: "toolCall", ID: "find", Name: "find", Arguments: map[string]any{"pattern": "*.txt"}},
		{Type: "toolCall", ID: "verify", Name: "read", Arguments: map[string]any{"path": "hello.txt"}},
	} {
	...
	final, err := ai.FauxAssistantMessage(ai.FauxAssistantText("文件已定位并读回。"))
	if err != nil {
		return err
	}
	...
	created, err := codingagent.CreateAgentSession(context.Background(), codingagent.CreateAgentSessionOptions{
		CWD: cwd, Model: &model, AgentDir: cwd, Tools: []string{"ls", "find", "read"}, StreamFunction: agent.StreamFunction(core.StreamSimple),
	})
	if err != nil {
		return err
	}
	defer created.Session.Dispose()
	if err := created.Session.Prompt(context.Background(), "查找文件并读回"); err != nil {
		return err
	}
