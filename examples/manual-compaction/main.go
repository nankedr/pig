...
func run() error {
	...
	dir, err := os.MkdirTemp("", "pig-manual-compaction-")
	if err != nil {
		return err
	}
	...
	settings, err := codingagent.NewInMemorySettingsManager(codingagent.Settings{Compaction: &codingagent.CompactionSettings{ReserveTokens: 1000, KeepRecentTokens: 10}})
	if err != nil {
		return err
	}
	...
	result, err := created.Session.Compact(ctx, "保留接口兼容约束")
	if err != nil {
		return err
	}
