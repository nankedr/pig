...
const outcome=JSON.parse(execFileSync('python3',['parity/terminal/model-selection.py',pi],{encoding:'utf8'}));
const c={schema_version:'1.0.0',id:'cli/interactive/model-selection',catalog_id:'contract:codingagent/model-selection',surface:'cli',input:{steps:['generate','model search pro + select','cycle thinking','scope flash + save + close','cycle single model','generate','cancel selector','generate','exit + reopen Session + generate'],terminal:{rows:32,columns:100}},observe:['outcome','side_effects']};
const observation={outcome,side_effects:[]};
const fixture={schema_version:'1.0.0',deterministic:true,baseline_id:lock.baseline_id,baseline_commit:lock.upstream.commit,upstream:{repository:lock.upstream.repository,commit:lock.upstream.commit,reference:'packages/coding-agent/src/modes/interactive/interactive-mode.ts'},case:c,observation,input_hash:caseDigest(c),observation_hash:observationDigest(observation),execution_method:'node parity/oracle/model-selection-cli.mjs <locked-pi-checkout>',platform:'posix',environment:{terminal:'xterm-256color',transport:'real child process + PTY + loopback SSE'}};
const path='parity/oracle/fixtures/model-selection-cli.json';
