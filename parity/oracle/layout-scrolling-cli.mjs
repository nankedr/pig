...
const outcome=JSON.parse(execFileSync('python3',['parity/terminal/layout-scrolling.py',pi],{encoding:'utf8'}));
const c={schema_version:'1.0.0',id:'cli/interactive/layout-scrolling',catalog_id:'contract:tui/layout-scrolling',surface:'cli',input:{terminal:{rows:24,columns:60},scenario:'regular/fullscreen, gated long response, home, append while reading, end, wheel, repeated resize and editor'},observe:['outcome','side_effects']};
const observation = {outcome,side_effects:[]};
const fixture = {schema_version:'1.0.0',deterministic:true,baseline_id:lock.baseline_id,baseline_commit:lock.upstream.commit,upstream:{repository:lock.upstream.repository,commit:lock.upstream.commit,reference:'packages/coding-agent/src/modes/interactive/interactive-mode.ts'},case:c,observation,input_hash:caseDigest(c),observation_hash:observationDigest(observation),execution_method:'node parity/oracle/layout-scrolling-cli.mjs <locked-pi-checkout>',platform:'posix',environment:{terminal:'xterm-256color',transport:'real child process + PTY + gated loopback SSE'}};
const path='parity/oracle/fixtures/layout-scrolling-cli.json';
