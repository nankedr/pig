...
const outcome=JSON.parse(execFileSync('python3',['parity/terminal/settings.py',pi],{encoding:'utf8'}));
const c={schema_version:'1.0.0',id:'cli/interactive/settings-cli',catalog_id:'contract:codingagent/interactive-settings',surface:'cli',input:{steps:['toggle each delivered interaction setting','restart','verify each effective menu value','exit'],terminal:{rows:40,columns:110}},observe:['outcome','side_effects']};
const observation={outcome,side_effects:[]};
const fixture={schema_version:'1.0.0',deterministic:true,baseline_id:lock.baseline_id,baseline_commit:lock.upstream.commit,upstream:{repository:lock.upstream.repository,commit:lock.upstream.commit,reference:'packages/coding-agent/src/modes/interactive/interactive-mode.ts'},case:c,observation,input_hash:caseDigest(c),observation_hash:observationDigest(observation),execution_method:'node parity/oracle/settings-cli.mjs <locked-pi-checkout>',platform:'posix',environment:{terminal:'xterm-256color',transport:'real child process + PTY; offline; no prompts'}};
const path='parity/oracle/fixtures/settings-cli.json';
