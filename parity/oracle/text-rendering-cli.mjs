...
const outcome=JSON.parse(execFileSync('python3',['parity/terminal/text-rendering.py',pi],{encoding:'utf8'}));
const c={schema_version:'1.0.0',id:'cli/interactive/text-rendering',catalog_id:'contract:codingagent/text-rendering',surface:'cli',input:{terminal:{rows:40,columns:44},scenario:'gated thinking, partial bash arguments, execution progress, final output and expansion'},observe:['outcome','side_effects']};
const observation = {outcome,side_effects:[]};
const fixture = {schema_version:'1.0.0',deterministic:true,baseline_id:lock.baseline_id,baseline_commit:lock.upstream.commit,upstream:{repository:lock.upstream.repository,commit:lock.upstream.commit,reference:'packages/coding-agent/src/modes/interactive/interactive-mode.ts'},case:c,observation,input_hash:caseDigest(c),observation_hash:observationDigest(observation),execution_method:'node parity/oracle/text-rendering-cli.mjs <locked-pi-checkout>',platform:'posix',environment:{terminal:'xterm-256color',transport:'real child process + PTY + gated loopback SSE'}};
const path='parity/oracle/fixtures/text-rendering-cli.json';
