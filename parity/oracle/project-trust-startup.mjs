...
const defaultOutput = join(here, "fixtures", "project-trust-startup.json");
...
async function main(){
 ...
 const dir=mkdtempSync(join(tmpdir(),'pi-project-trust-startup-')),agentDir=join(dir,'agent'),cwd=join(dir,'project');mkdirSync(agentDir);mkdirSync(cwd);
 const requests=[];
 ...
 try{
  ...
  const configured=join(dir,'configured'),project=join(dir,'project-sessions');
  mkdirSync(join(cwd,'.pi'));writeFileSync(join(cwd,'AGENTS.override.md'),'CONTEXT_VISIBLE_WITHOUT_TRUST');
  writeFileSync(join(cwd,'AGENTS.md'),'SHADOWED_CONTEXT');
  writeFileSync(join(cwd,'.pi','SYSTEM.md'),'TRUST_SENSITIVE_SYSTEM');
  writeFileSync(join(agentDir,'settings.json'),JSON.stringify({defaultProvider:'deepseek',defaultModel:'deepseek-v4-flash',sessionDir:configured}));
  writeFileSync(join(cwd,'.pi','settings.json'),JSON.stringify({defaultModel:'deepseek-v4-pro',sessionDir:project}));
  const cases=[];
  for(const [name,extra] of [['ask',[]],['approve',['--approve']],['deny',['--no-approve']],['no-context',['--no-approve','--no-context-files']]]) {
   // Explicit Session path isolates the Pi pre-trust sessionDir defect from the shared behavior case.
   await run(['--session-dir',configured,...extra]);
   const request=requests.at(-1),prompt=JSON.stringify(request.messages);
   cases.push({name,model:request.model,context:prompt.includes('CONTEXT_VISIBLE_WITHOUT_TRUST'),shadowed:prompt.includes('SHADOWED_CONTEXT')});
  }
  const input={projection:'Real locked Pi Headless CLI: project trust ask/approve/deny, project model override and Context File exception. Explicit session directory excludes the ADR-0010 pre-trust path deviation. SYSTEM.md execution belongs to M5.'};
  const observation={outcome:{cases},side_effects:[]};
  const c={schema_version:'1.0.0',id:'cli/pig/project-trust-startup',catalog_id:'contract:security/project-trust',surface:'cli',input,observe:['outcome','side_effects']};
  const fixture={schema_version:'1.0.0',deterministic:true,baseline_id:lock.baseline_id,baseline_commit:lock.upstream.commit,upstream:{repository:lock.upstream.repository,commit:lock.upstream.commit,reference},case:c,observation,input_hash:caseDigest(c),observation_hash:observationDigest(observation),execution_method:'node --experimental-strip-types parity/oracle/project-trust-startup.mjs <locked-pi-checkout>',platform:'any',environment:{node:process.version,oracle_entry:reference}};
