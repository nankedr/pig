...
const outcome=JSON.parse(execFileSync('python3',['parity/terminal/bash.py',pi],{encoding:'utf8'}));
const c={schema_version:'1.0.0',id:'cli/interactive/bash-cli',catalog_id:'contract:codingagent/interactive-bash',surface:'cli',input:{steps:['included/excluded stdout/stderr and exit','Bash during generation and follow-up','cancel with partial output','truncate with full output file','subsequent Provider context','exit'],terminal:{rows:40,columns:120}},observe:['outcome','side_effects']};
const observation={outcome,side_effects:[]};
const fixture={schema_version:'1.0.0',deterministic:true,baseline_id:lock.baseline_id,baseline_commit:lock.upstream.commit,upstream:{repository:lock.upstream.repository,commit:lock.upstream.commit,reference:'packages/coding-agent/src/modes/interactive/interactive-mode.ts'},case:c,observation,input_hash:caseDigest(c),observation_hash:observationDigest(observation),execution_method:'node parity/oracle/bash-cli.mjs <locked-pi-checkout>',platform:'posix',environment:{terminal:'xterm-256color',transport:'real child process + PTY; offline controlled SSE and real Bash'}};
const path='parity/oracle/fixtures/bash-cli.json';
