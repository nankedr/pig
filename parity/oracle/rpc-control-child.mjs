...
models.registerProvider("deepseek",{api:"openai-completions",apiKey:"fixture",baseUrl:"https://invalid.test",models:[models.getModel("deepseek","deepseek-v4-flash"),models.getModel("deepseek","deepseek-v4-pro")]});
...
session.agent.streamFunction = (_model, _context, options) => {
 ...
 const message = {role:"assistant",content:[{type:"text",text:"rpc answer"}],api:"openai-completions",provider:"deepseek",model:"deepseek-v4-flash",usage:{input:10,output:2,cacheRead:0,cacheWrite:0,totalTokens:12,cost:{input:0,output:0,cacheRead:0,cacheWrite:0,total:0}},stopReason:"stop",timestamp:1};
 ...
 const held=++calls===1;
 const timer = setInterval(()=>{if(held&&!existsSync(join(dir,"release")))return;clearInterval(timer);stream.push({type:"text_end",contentIndex:0,content:message.content[0].text,partial:message});stream.push({type:"done",reason:"stop",message});},10);
