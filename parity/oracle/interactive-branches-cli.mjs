...
const outcome=JSON.parse(execFileSync('python3',['parity/terminal/interactive-branches.py',pi],{encoding:'utf8'}));
const c={schema_version:'1.0.0',id:'cli/interactive/interactive-branches',catalog_id:'contract:codingagent/interactive-branches',surface:'cli',input:{steps:['cancel fork','fork latest user + submit draft','navigate user + submit draft','label ancestor + cancel summary','navigate ancestor without summary + generate','navigate old branch with summary + generate','reopen fork + generate'],terminal:{rows:32,columns:100}},observe:['outcome','side_effects']};
const observation={outcome,side_effects:[]};
const fixture={schema_version:'1.0.0',deterministic:true,baseline_id:lock.baseline_id,baseline_commit:lock.upstream.commit,upstream:{repository:lock.upstream.repository,commit:lock.upstream.commit,reference:'packages/coding-agent/src/modes/interactive/interactive-mode.ts'},case:c,observation,input_hash:caseDigest(c),observation_hash:observationDigest(observation),execution_method:'node parity/oracle/interactive-branches-cli.mjs <locked-pi-checkout>',platform:'posix',environment:{terminal:'xterm-256color',transport:'real child process + PTY + loopback SSE'}};
const path='parity/oracle/fixtures/interactive-branches-cli.json';
