...
with tempfile.TemporaryDirectory(prefix='pi-interactive-') as root:
    ...
    cases = json.loads(pathlib.Path(__file__).with_name('keys-cli-cases.json').read_text())
    (agent/'keybindings.json').write_text(json.dumps(cases['config']))
    ...
    try:
        ...
        time.sleep(.5)
        cases = json.loads(pathlib.Path(__file__).with_name('keys-cli-cases.json').read_text())
        os.write(master,('discard'+cases['clear']).encode())
        for chunk in cases['negotiation']:
            os.write(master,chunk.encode());time.sleep(.03)
        time.sleep(.1)
        for index, turn in enumerate(cases['turns']):
            ...
            time.sleep(.1)
        os.write(master,cases['exit'].encode())
        ...
        print(json.dumps({'prompts':prompts,'user_counts':[sum(m['role']=='user' for m in r['messages']) for r in requests],'exit_code':process.returncode}))
    finally:
        release.set()
