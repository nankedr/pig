...
with tempfile.TemporaryDirectory(prefix='pi-interactive-') as root:
    root = pathlib.Path(root); agent = root/'.pi/agent'; agent.mkdir(parents=True); sessions = root/'sessions'
    cases = json.loads(pathlib.Path(__file__).with_name('autocomplete-cli-cases.json').read_text())
    for name, content in cases['files'].items():
        path=root/name; path.parent.mkdir(parents=True,exist_ok=True); path.write_text(content)
    for name in cases.get('executables',[]): (root/name).chmod(0o755)
    ...
    cmd += [arg.replace('$ROOT',str(root)) for arg in cases['args']]
    ...
    def pump(seconds):
        ...
        while time.monotonic()<end:
            if select.select([master],[],[],min(.02,max(0,end-time.monotonic())))[0]: output += os.read(master,65536)
    try:
        ...
        for index, turn in enumerate(cases['turns']):
            ...
            for chunk_index, text in enumerate(chunks):
                offset=len(output)
                os.write(master,text.replace('$ROOT',str(root)).encode())
                marker=(turn.get('waits') or ['']*len(chunks))[chunk_index]
                if marker: wait_for(lambda: marker.encode() in output[offset:])
                pump(cases['delayMS']/1000)
        ...
        for request in requests:
            ...
            prompts.append(content.replace(str(root),'$ROOT'))
