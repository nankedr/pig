...
requests=[]; release=threading.Event(); summary_mode='success'
def text(m):
...
with tempfile.TemporaryDirectory(prefix='maintenance124-') as tmp:
 root=pathlib.Path(tmp); agent=root/'agent'; agent.mkdir(); sessions=root/'sessions'
 (agent/'settings.json').write_text(json.dumps({'quietStartup':True,'tuiMode':'regular','compaction':{'enabled':False,'keepRecentTokens':10,'reserveTokens':1000},'retry':{'enabled':False}}))
 ...
 try:
  ...
  command('first question with enough context for compaction','RESPONSE_1')
