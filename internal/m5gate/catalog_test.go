...
func TestM5CatalogEvidence(t *testing.T) {
	...
	for _, entry := range entries {
		if entry.Milestone != "M5" && entry.ID != "cmd-pig" && entry.ID != "module-codingagent" && entry.ID != "contract:cli/pig/args" {
			continue
		}
		...
		if !strings.HasPrefix(entry.ID, "contract:") || entry.ID == "contract:migration/auth-and-layout" {
			continue
		}
	...
	sort.Strings(scope)
	want, err := os.ReadFile(filepath.Join(root, "internal/m5gate/testdata/catalog_scope.txt"))
	if err != nil {
		t.Fatal(err)
	}
	if strings.Join(scope, "\n")+"\n" != string(want) {
		t.Error("M5 mapping, status or exact partial scope changed; audit before updating freeze snapshot")
	}
}
