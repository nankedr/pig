...
func assertCodingAgentConstructorMetadata(t *testing.T, entriesByID map[string][]catalog.Entry, symbol surface.Symbol, entry catalog.Entry) {
	...
	case catalog.StatusPartial:
		reviewed := entry.ID == "constructor:codingagent/src/modes/interactive/components/bash-execution.ts#BashExecutionComponent" || entry.ID == "constructor:codingagent/src/core/resource-loader.ts#DefaultResourceLoader" || entry.ID == "constructor:codingagent/src/core/keybindings.ts#KeybindingsManager" || entry.ID == "constructor:codingagent/src/modes/interactive/components/assistant-message.ts#AssistantMessageComponent" || entry.ID == "constructor:codingagent/src/modes/interactive/components/tool-execution.ts#ToolExecutionComponent" || entry.ID == "constructor:codingagent/src/modes/interactive/components/user-message.ts#UserMessageComponent"
	...
	default:
		t.Errorf("Coding Agent constructor %s status = %q, want scaffolded, inventoried or reviewed partial", entry.ID, entry.Status)
	}
}
