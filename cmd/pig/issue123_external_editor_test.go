...
func TestPigExternalEditorParity123(t *testing.T) {
	...
	fixture, err := parity.LoadFixture("../../parity/oracle/fixtures/external-editor-cli.json", locked)
	if err != nil {
		t.Fatal(err)
	}
	...
	for _, mode := range []string{"regular", "fullscreen"} {
		t.Run(mode, func(t *testing.T) {
			cmd := exec.Command("python3", "../../parity/terminal/external-editor.py", binary, "--pig", "--"+mode)
			data, err := cmd.Output()
			if err != nil {
