...
func TestPigSettingsParity121(t *testing.T) {
	...
	cmd := exec.Command("python3", "../../parity/terminal/settings.py", binary, "--pig")
	data, err := cmd.Output()
	if err != nil {
	...
	fixture, err := parity.LoadFixture("../../parity/oracle/fixtures/settings-cli.json", locked)
	if err != nil {
		t.Fatal(err)
	}
