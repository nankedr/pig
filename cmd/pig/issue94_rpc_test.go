...
func TestRPC94AutomaticCompaction(t *testing.T) {
	binary := buildPigBinary(t)
	cwd, settingsDir, sessionsDir := t.TempDir(), t.TempDir(), t.TempDir()
	manager, err := codingagent.NewSessionManager(cwd, &sessionsDir)
	if err != nil {
		t.Fatal(err)
	}
	for _, text := range []string{"old request", "recent request long enough to keep"} {
		if _, err := manager.AppendMessage(ai.UserMessage{Role: ai.MessageRoleUser, Content: ai.UserText(text)}); err != nil {
			t.Fatal(err)
		}
		if _, err := manager.AppendMessage(ai.AssistantMessage{Role: ai.MessageRoleAssistant, Content: []ai.AssistantContent{ai.TextContent{Type: ai.ContentTypeText, Text: "seed answer"}}, API: "openai-completions", Provider: "deepseek", Model: "deepseek-v4-flash", StopReason: ai.StopReasonStop, Timestamp: 1}); err != nil {
			t.Fatal(err)
		}
	}
	if err := os.WriteFile(filepath.Join(settingsDir, "settings.json"), []byte(`{"compaction":{"enabled":true,"reserveTokens":100,"keepRecentTokens":10},"retry":{"enabled":true,"maxRetries":2,"baseDelayMs":1}}`), 0600); err != nil {
		t.Fatal(err)
	}
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		text := "recovered"
		if bytes.Contains(request, []byte("You are a context summarization assistant.")) {
			summaries++
			if summaries == 1 {
				http.Error(w, `{"error":{"message":"503 unavailable"}}`, 503)
				return
			}
			text = "checkpoint"
		} else {
			generations++
			if generations == 1 {
				http.Error(w, `{"error":{"message":"prompt is too long"}}`, 400)
				return
			}
		}
		w.Header().Set("Content-Type", "text/event-stream")
		fmt.Fprintf(w, "data: {\"id\":\"rpc\",\"model\":\"deepseek-v4-flash\",\"choices\":[{\"delta\":{\"content\":%q},\"finish_reason\":\"stop\"}]}\n\ndata: [DONE]\n\n", text)
	}))
	defer server.Close()
	client := codingagent.NewRPCClient(codingagent.RPCClientOptions{CLIPath: &binary, CWD: &cwd, Args: []string{"--provider", "deepseek", "--model", "deepseek-v4-flash", "--api-key", "fixture", "--session", *manager.GetSessionFile(), "--no-tools", "--no-context-files", "--offline"}, Env: map[string]string{"PIG_CODING_AGENT_DIR": settingsDir, "PIG_DEEPSEEK_BASE_URL": server.URL}})
	...
	defer client.Stop(context.Background())
	events, err := client.PromptAndWait(ctx, "go")
	if err != nil {
		t.Fatal(err)
	}
	kinds := map[codingagent.AgentSessionEventType]bool{}
	for _, event := range events {
		kinds[event.AgentSessionEventType()] = true
		if end, ok := event.(*codingagent.AgentSessionCompactionEndEvent); ok && (end.Aborted || !end.WillRetry || end.Result == nil) {
			t.Fatalf("compaction end: %+v", end)
		}
	}
	for _, kind := range []codingagent.AgentSessionEventType{codingagent.AgentSessionEventTypeCompactionStart, codingagent.AgentSessionEventTypeCompactionEnd, codingagent.AgentSessionEventTypeSummarizationRetryScheduled, codingagent.AgentSessionEventTypeSummarizationRetryAttemptStart, codingagent.AgentSessionEventTypeSummarizationRetryFinished} {
		if !kinds[kind] {
			t.Errorf("missing %s in %v", kind, kinds)
		}
	}
	text, err := client.GetLastAssistantText(ctx)
	if err != nil || text == nil || *text != "recovered" {
		t.Fatalf("recovered text: %v %v", text, err)
	}
}

func TestRPC94PreflightCompactionAbort(t *testing.T) {
	binary := buildPigBinary(t)
	cwd, settingsDir, sessionsDir := t.TempDir(), t.TempDir(), t.TempDir()
	manager, err := codingagent.NewSessionManager(cwd, &sessionsDir)
	if err != nil {
	...
	for _, text := range []string{"old request", "recent request long enough to keep"} {
		...
		if _, err := manager.AppendMessage(ai.AssistantMessage{Role: ai.MessageRoleAssistant, Content: []ai.AssistantContent{ai.TextContent{Type: ai.ContentTypeText, Text: "seed answer"}}, API: "openai-completions", Provider: "deepseek", Model: "deepseek-v4-flash", StopReason: ai.StopReasonStop, Timestamp: 1, Usage: ai.Usage{Input: 2000000, TotalTokens: 2000000}}); err != nil {
			t.Fatal(err)
		}
	}
	if err := os.WriteFile(filepath.Join(settingsDir, "settings.json"), []byte(`{"compaction":{"enabled":true,"reserveTokens":100,"keepRecentTokens":10}}`), 0600); err != nil {
		t.Fatal(err)
	}
	...
	unsub, err := client.OnEvent(func(event codingagent.JSONAgentSessionEvent) {
		if event.AgentSessionEventType() == codingagent.AgentSessionEventTypeCompactionStart {
			compacting <- struct{}{}
		}
	})
	if err != nil {
	...
	go func() { prompt <- client.Prompt(ctx, "must not be accepted") }()
	select {
	case <-compacting:
	case <-ctx.Done():
		t.Fatal("preflight never compacted")
	}
	if err := client.Abort(ctx); err != nil {
		t.Fatal(err)
	}
	...
	case err := <-prompt:
		if err == nil {
			t.Error("prompt reported success before canceled preflight completed")
		}
	case <-ctx.Done():
		t.Fatal("prompt waiter leaked after abort")
	}
	messages, err := client.GetMessages(ctx)
	if err != nil || len(messages) != 4 {
		t.Fatalf("preflight cancellation changed history: %d %v", len(messages), err)
	}
}
