...
func issue87ConfigurationCatalogEntry() catalog.Entry {
	return catalog.Entry{
		...
		Partial:   &catalog.Partial{Supported: []string{"public SDK model set/cycle, available/scoped snapshot filtering, thinking set/cycle/capability levels and preference restoration", "dynamic authentication and subsequent runtime requests, active Tool registry restrictions with prompt rebuild, v3 model/thinking restore", "busy rejection, ordered thinking notifications, immutable Session snapshots and persistence failure rollback"}, Unsupported: []string{"extension model/thinking hooks and active-tool changes within a running turn, TUI controls, full ToolDefinition metadata and custom extension registry remain deferred; RPC model/thinking controls are covered by contract:rpc/session-control; fixed Pi has no active-Tool RPC command", "runtime execution remains limited to existing DeepSeek/OpenAI Chat Completions Adapter; no models.json overlays or dynamic Provider registration", "cross-file crash atomicity and storage backends that partially write before returning an error are outside the existing persistence guarantee"}},
