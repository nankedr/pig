...
func decodeRPCEvent(raw []byte) (JSONAgentSessionEvent, error) {
	...
	case AgentSessionEventTypeCompactionStart:
		event = &AgentSessionCompactionStartEvent{}
	case AgentSessionEventTypeCompactionEnd:
		event = &AgentSessionCompactionEndEvent{}
	case AgentSessionEventTypeSummarizationRetryScheduled:
		event = &AgentSessionSummarizationRetryScheduledEvent{}
	case AgentSessionEventTypeSummarizationRetryAttemptStart:
		var source SummarizationRetrySource
		if err := json.Unmarshal(fields["source"], &source); err != nil {
			return nil, err
		}
		if source == SummarizationRetrySourceBranchSummary {
			event = &AgentSessionBranchSummaryRetryAttemptStartEvent{}
		} else {
			event = &AgentSessionCompactionRetryAttemptStartEvent{}
		}
	case AgentSessionEventTypeSummarizationRetryFinished:
		event = &AgentSessionSummarizationRetryFinishedEvent{}
	case AgentSessionEventTypeBashExecutionUpdate:
		event = &AgentSessionBashExecutionUpdateEvent{}
