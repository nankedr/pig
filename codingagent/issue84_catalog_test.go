...
func issue84FindLsEvidence(t *testing.T) []issue32ModuleEvidenceDescriptor {
	...
	for _, e := range []struct{ kind, path, ref, id, run string }{
		...
		{"go-test", "codingagent/issue84_fd_unix_test.go", "codingagent/issue84_fd_unix_test.go#TestFindLsFDCancellationWaitsForExit", "issue84-find-ls-platform", "go test -race ./codingagent -run '^TestFindLsFD' -count=1"},
