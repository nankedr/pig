...
func issue81ToolsCatalogEntry() catalog.Entry {
	return catalog.Entry{
		...
		Notes:     "Issue #81 verifies M3.11 through the highest public Session and process boundaries. AgentTools injection and explicit in-memory managers retain the M1/M2 SDK path. Chat Completions accepts SessionID only with explicit CacheRetentionNone; no cache key or affinity header is sent. Pi selection Oracle is source-locked. Issue #84 adds explicit find/ls under contract:codingagent/find-ls. No M3-wide freeze claim.",
	}
}
