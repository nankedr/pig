...
func issue74SettingsCatalogEntry() catalog.Entry {
	return catalog.Entry{
		...
		Partial:   &catalog.Partial{Supported: []string{"global file and in-memory SettingsManager defaults, deep overrides, migrations, locked field-level saves, reload, flush, error collection and preservation of sparse/unknown data", "Headless CLI and SDK use saved Provider/model/thinking and Session directory; real processes match the locked Pi startup precedence fixture", "Issue #75 loads project settings only after trust and deep-merges them with global settings; revocation restores global configuration"}, Unsupported: []string{"project writes remain unavailable; project reads require trust as implemented by issue #75", "future resource/package/proxy activation remains an explicit Capability Stub; other future settings are data only", "built-in Provider support remains the existing DeepSeek runtime; broader Provider adapters and model catalogs stay deferred"}},
		Deviation: &catalog.Deviation{ADR: "docs/adr/0010-trust-and-host-security.md", Reason: "Project settings are never read before trust, including sessionDir; explicit ProjectTrusted opts in to the project scope."},
		Notes:     "Issue #74 implements global settings and issue #75 adds gated project settings, deep merge and trust revocation for Headless sessions. Go setters complete writes synchronously under a directory lock; Flush waits for concurrent setters and DrainErrors reports scoped failures. Overrides reset on save/reload. Pig reads only Pig-owned global and authorized project state and never starts analytics, package installation, extensions, or proxy services from stored data.",
	}
}
