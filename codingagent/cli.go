...
Modes:
  ...
  rpc          --mode rpc; local JSONL prompt, events, state/messages, and abort
...
Current Headless Availability:
  ...
  support resume and fork; other Tools and extensions
  ...
  RPC uses stdin for JSONL commands and stdout for responses/events;
  images and commands beyond basic conversation remain unavailable.
...
Examples:
  ...
  # Stream a v3 Session header followed by AgentSessionEvent JSONL
  pig --provider deepseek --model deepseek-v4-flash --mode json "Explain this package"
...
Built-in Tool Names:
  ...
  find   - Find files by glob pattern (requires fd; opt in with --tools find)
  ls     - List directory contents (opt in with --tools ls)

Exit Status:
  0    Static success, successful Headless run, or JSON-encoded Provider failure
  1    Invalid arguments, text-mode Provider failure, or unavailable capability
  130  A Headless run was canceled (Unix: SIGINT, SIGTERM, SIGHUP)
`
...
func StaticCLIContract() CLIContract {
	return CLIContract{
		...
		ExitStatus: "0 for static help/version, successful Headless runs, or JSON-encoded Provider failures; 1 for argument, text-mode Provider, and unavailable-capability errors; 130 for interrupted Headless runs",
	}
}
