...
func TestInteractiveThemesCatalog120(t *testing.T) {
	...
	for _, item := range []struct{ kind, path, run string }{
		...
		{"go-test", "codingagent/issue120_interactive_test.go", "go test -race ./codingagent -run 'TestInteractive(ThemeNotifications|FixedToAutomaticTheme|InvalidThemeSessionReplacement)120' -count=1"},
		...
		{"go-test", "codingagent/issue120_watch_test.go", "go test -race ./codingagent -run TestThemeWatchDebounce120 -count=1"},
