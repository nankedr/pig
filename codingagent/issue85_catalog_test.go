...
func issue85MessagesCatalogEntry() catalog.Entry {
	return catalog.Entry{
		...
		Partial: &catalog.Partial{Supported: []string{"Issue #102 adds local template expansion; evidence: contract:codingagent/prompt-templates", "text SendUserMessage normalization and idle execution, running DeliverAs and Prompt StreamingBehavior routing to Legacy Agent", "Steer/FollowUp admission, FIFO one/all modes with saved settings, owned queue queries, pending count and ClearQueue", "queue_update before user message_start, message_end persistence and reopen without duplicate consumed history; cancellation retention and concurrent final-turn admission"}, Unsupported: []string{"images, skill expansion, extension input/commands/runtime, PreflightResult and Source remain exact capability boundaries", "manual-compaction queues, new enqueue during Provider retry backoff, interactive UI; text RPC delivery is covered by contract:rpc/session-control; automatic-compaction queues are covered by contract:codingagent/compaction; queued but unconsumed messages are in-memory only",
