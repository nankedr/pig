...
func issue85MessagesCatalogEntry() catalog.Entry {
	return catalog.Entry{
		...
		Partial:   &catalog.Partial{Supported: []string{"text SendUserMessage normalization and idle execution, running DeliverAs and Prompt StreamingBehavior routing to Legacy Agent", "Steer/FollowUp admission, FIFO one/all modes with saved settings, owned queue queries, pending count and ClearQueue", "queue_update before user message_start, message_end persistence and reopen without duplicate consumed history; cancellation retention and concurrent final-turn admission"}, Unsupported: []string{"images, template/skill expansion, extension input/commands/runtime, PreflightResult and Source remain exact capability boundaries", "compaction/retry queues, RPC and interactive UI; queued but unconsumed messages are in-memory only", "ClearQueue retains its published error-only Go signature; queues manipulated directly through Session.Agent are outside Session display tracking"}},
