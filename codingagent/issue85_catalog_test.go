...
func issue85MessagesCatalogEntry() catalog.Entry {
	return catalog.Entry{
		...
		Partial: &catalog.Partial{Supported: []string{"text SendUserMessage normalization and idle execution, running DeliverAs and Prompt StreamingBehavior routing to Legacy Agent", "Steer/FollowUp admission, FIFO one/all modes with saved settings, owned queue queries, pending count and ClearQueue", "queue_update before user message_start, message_end persistence and reopen without duplicate consumed history; cancellation retention and concurrent final-turn admission"}, Unsupported: []string{"images, template/skill expansion, extension input/commands/runtime, PreflightResult and Source remain exact capability boundaries", "manual-compaction queues, new enqueue during Provider retry backoff, RPC and interactive UI; automatic-compaction queues are covered by contract:codingagent/compaction; queued but unconsumed messages are in-memory only",
